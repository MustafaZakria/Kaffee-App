            ...
            if (currentDestination?.route in bottomNavRoutes) {
                CompositionLocalProvider(
                    ...
                    NavigationBar(
                        ...
                        modifier = Modifier.heightIn(max = dimensionResource(id = R.dimen.main_bottom_navigation_height))
                    ) {
                        ...
                        bottomNavItems.forEach { item ->
                            NavigationBarItem(
                                ...
                                icon = {
                                    BadgedBox(badge = {
                                        if (item.hasUpdate.value) Badge()
                                    }) {
                                        Icon(imageVector = if (currentDestination?.hierarchy?.any { it.route == item.route } == true) ImageVector.vectorResource(
                                            id = item.selectedIcon
                                        )
                                        else ImageVector.vectorResource(id = item.unselectedItem),
